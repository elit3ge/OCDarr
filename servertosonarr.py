...
ACTION_ONLY = config.get('action_only', [])
...
def get_server_activity():
    ...
    try:
        ...
        return username, series_title, season_number, episode_number
...
def main():
    username, series_name, season_number, episode_number = get_server_activity()
    if series_name:
        # Check if the series is in the ACTION_ONLY list if it's specified
        if ACTION_ONLY and series_name not in ACTION_ONLY:
            logging.info(f"Series '{series_name}' is not in the action_only list. Skipping...")
            return
